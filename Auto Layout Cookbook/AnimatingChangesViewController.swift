...
class AnimatingChangesViewController: UIViewController {
  ...
  override func viewDidAppear(_ animated: Bool) {
    ...
    timer = Timer.scheduledTimer(timeInterval: 1.0, target: self, selector: #selector(updateViews(timer:)), userInfo: nil, repeats: false)
  }
  ...
  @objc func updateViews(timer: Timer) {
    let margins = view.layoutMarginsGuide
    
    guard let leaving = bottomView else { return }
    ...
    UIView.animate(withDuration: 0.25, animations: { [unowned self] in
      ...
      Timer.scheduledTimer(timeInterval: 1.0, target: self, selector: #selector(self.updateViews(timer:)), userInfo: nil, repeats: false)
    })
  }
